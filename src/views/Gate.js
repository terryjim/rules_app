...
import EditGateForm from '../forms/EditGateForm'
...
class Gate extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.closeModal)    //保存成功后关闭表单窗口
      this.setState({ showEditGate: false })
  }
  ...
  toggleShowEditGate = () => {
    this.setState({
      showEditGate: !this.state.showEditGate,
    });
  }
  //切换错误窗口状态（开、闭）  
  toggleShowDanger = () => {
      ...
      <a className="fa fa-pencil" style={{ fontSize: 15, color: '#00adff', alignItems: 'top' }}
        onClick={
          (e) => {
            ...
            this.setState({ showEditGate: true, edit: true })
          }
        }>
      </a>
      &nbsp;
     <a className="fa fa-remove" style={{ fontSize: 15, color: '#FF5722', alignItems: 'top' }}
        onClick={
          e => {
            e.stopPropagation()
            this.setState({ selection: [props.row.id] })
            this.props.dispatch(showConfirm('是否删除选中记录？', 'gate', 'del'))
          }
        }>
      </a>
    </div>)
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn" style={{ marginTop: '-15px' }}>
          ...
          <Button color="danger"
            ...
            onClick={() => {
              if (this.state.selection.length < 1)
                ...
                this.props.dispatch(showConfirm('是否删除选中的' + this.state.selection.length + '条记录？', 'gate', 'del'));
            }}>
            <i className="fa fa-remove" ></i>&nbsp;删除
      </Button>
        </div>
        <CheckboxTable ref={r => (this.checkboxTable = r)} keyField='id' data={gates.content}
          pages={gates.totalPages} columns={this.columns} defaultPageSize={window.TParams.defaultPageSize} filterable
          className="-striped -highlight"
          total={gates.totalElements}
            ...
            (state, rowInfo, column, instance) => {
              ...
              return {
                style, onDoubleClick: (e, handleOriginal) => {
                  ...
                  this.setState({ showEditGate: true, edit: false })
                },
        ...
        <TopModal style={{ "max-width": "850px" }} isOpen={this.state.showEditGate} toggle={() => this.toggleShowEditGate()}
          className={'modal-primary ' + this.props.className}>
          <ModalHeader toggle={() => this.toggleShowEditGate()}>企业信息</ModalHeader>
          <ModalBody>
            <EditGateForm readOnly={!this.state.edit} onSubmit={this.submit} closeForm={this.toggleShowEditGate} />
          </ModalBody>
        </TopModal>
      </div>
    )
...
const mapStateToProps = (state) => {
  let gates = state.cList
  let success = state.success

  console.log(gates)
  let editedIds = state.editedIds
  let confirmDel = state.confirm.module === 'gate' && state.confirm.operate === 'del' ? state.confirm.confirm : false
  return { closeModal: success.show, gates, editedIds, confirmDel }
}
